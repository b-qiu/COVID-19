...
case_sim <- function(source, delay, duration, new_cases, growth, k_1, k_2, bias){
  ...
  # k_2 <- 0.5
  # front_bias <- -12
  # last_date <- as.Date("2020/02/01")
  
  # finding the last date of actual data
  last_date <- as.Date(tail(source$dates,1))
  
  # date range of simulated data
  sim_date_range <- seq.Date(from = last_date + sim_start_tp + 1, 
                             length.out = sim_dur, 
                             by =  "days")
  x_1 <- seq(1, sim_dur, 1)

  # midpoint of the stiched curves
  ...
  p_g_r <- ggplot(sim_bind) +
    ...
    geom_vline(xintercept=sim_bind$dates[sim_bind$x==midpoint_1], 
               linetype="dashed", 
               color = "red") +
    scale_x_date(date_labels = "%b %d") +
    theme_bw() +
  ...
  fill_dates <- seq.Date(last_date, 
                         last_date + sim_start_tp - 1 + 2, 
                         "days")
